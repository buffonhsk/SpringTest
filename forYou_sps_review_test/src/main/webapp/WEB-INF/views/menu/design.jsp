...
<div class="wrap_paging">
	<ul class="pagination">
		<c:if test="${pageMaker.prev}">
			<li><a
				href="design${pageMaker.makeQuery(pageMaker.startPage - 1)}">%laquo;</a></li>
		</c:if>

		<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}"
			var="idx">
			<li <c:out value="${pageMaker.cri.page == idx?'class =active':''}"/>>
				<a href="design${pageMaker.makeQuery(idx)}">${idx}</a>
			</li>
		</c:forEach>

		<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
			<li><a
				href="design${pageMaker.makeQuery(pageMaker.endPage + 1) }">&raquo;</a></li>
		</c:if>
	</ul>
